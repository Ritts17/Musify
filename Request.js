...
function search(){
    ...
    songs.forEach(element => {
        console.log(element['songName'].includes(val));
        if(element['songName'].includes(val)){
            document.getElementById('songNotFound').style.display = 'none';
            document.getElementById('songitemSearch').style.display = 'block';
            document.getElementById('songImageSearch').src = element['coverPath'];
            document.getElementById('songNameSearch').textContent = element['songName'];
            let ind = songs.indexOf(element);
            console.log(ind);
            recentSearchIndex = ind;
            flag = 1;
        }
    });
    console.log(flag);
    if(flag == 0){
        document.getElementById('songNotFound').style.display = 'block';
    }
}

function playSearch(){
    let elem = document.getElementById('playSearch');
    elem.classList.remove('fa-play-circle');
    elem.classList.add('fa-pause-circle');
    
    if(recentSearchIndex != 999){
        audioElement.pause();
        console.log(songs[recentSearchIndex].filePath);
        audioElement.src = songs[recentSearchIndex].filePath;
        masterSongName.innerText = songs[recentSearchIndex].songName;
